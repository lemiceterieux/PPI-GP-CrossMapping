...
#        self.muMat = "fix" 
    ...
    def testStateSpaceCorrelation(self, X, Y, Mu, m=3, tau=1, cuda=0, inc=.7,ind=None):
        Z = []
        if not X is torch.Tensor:
            X = torch.from_numpy(X).float().cuda(cuda)
            ZZ = []
            for mm in Mu:
                ZZ += [torch.from_numpy(np.array(mm)).float().cuda(cuda)]
            for z in Y:
                Z += [torch.from_numpy(np.array(z)).float().cpu()]
        x = (X - X.mean())/X.std()
        mu = []
        for mm in ZZ:
            mu += [(mm - mm.mean())/mm.std()]
        y = []
        for z in Z:
            y += [((z - z.mean())/z.std())]
        ...
        muar = []
        for z in mu:
            muar += [torch.stack([z[:,tau*i:tau*i-tau*m] for i in range(m)],1).cpu()]
            muar[-1] = muar[-1].reshape(m,-1).T#-1,muar[-1].shape[-1]).T
            muar[-1] = muar[-1]
        muar = torch.stack(muar).cuda(cuda)

        yp = []
#        tempyar = []
#        if not (ind is None):
#            for k in range(len(yar)):
#                design = torch.cat([yar[i] for i in range(len(yar)) if i != ind and i !=k],-1)
#                W = torch.linalg.inv(design.transpose(-1,-2).matmul(design)).matmul(design.transpose(-1,-2)).matmul(yar[k])
#                tempyar += [yar[k] - design.matmul(W)]
#            yar = torch.stack(tempyar)

        # Train outs
        xp = xar[1:]
        xi = xar[:-1]#.matmul(simMats[0])
        mup = muar[:,1:]#.matmul(simMats[0])
        yp = yar[:,1:]#.matyl(simMats[0])
        yar = yar[:,:-1]#.matyl(simMats[0])
...
#        print(yar.shape)
        ...
        m, k= gpx.forward(xi, xp, yar.matmul(W),mup.matmul(W),yp.matmul(W),ind)#.matmul(s))
        mx = m.transpose(-1,-2)
        if len(k.shape) > 1:
            kx = k.transpose(-1,-2)
        else:
    ...
    def optimizeHyperparms(self, data, inp,mu, lr=.001, ld=1., sigma=6., noise=.1, niter=20, ns=False, kernel=None, m=40):
        ...
        eld = (ld*torch.ones(inp.shape[-1])).float().cuda(self.cuda).requires_grad_()
        ...
        for i in range(niter):
            ...
            for k in range(mSamples):
                ...
                trr = train.matmul(V)
                mrr = mu.matmul(V)
                sqtrmr = (noise**2)*(1-torch.exp(-(1/eld**2*(trr-mrr)**2)).mean(1))
        ...
        self.eld = eld.detach().requires_grad_(False)
    ...
    # Posterior Inference
    def forward(self, inp, observations, test,mu,testp, ind=None, ld=1, sig=10, noise=.1, target="squaredexp"):
        # Choose kernel
        ...
        if self.muMat is None:
            ...
            lp, parms, muMatrix, stdMatrix = self.optimizeHyperparms(observations, inp,mu[ind].squeeze(), sigma=sigma, noise=noise, ld=ld ,niter=80,lr=1e-4, kernel=kernel)
            self.sigma = parms[1]
            self.ld = parms[0]
            self.noise = parms[2]
        ...
        ltestp = observations.matmul(self.A)
        mus = mu.matmul(self.A)
        sqtrmr = []
        temp = (self.noiseC**2)*(1-torch.exp(-(1/self.eld**2*(ltestp-mus)**2).mean(-1)))
        for i in range(len(mus)):
            mask = np.arange(len(temp))
            mask = np.logical_and(mask!=ind,mask!=i)
            sqtrmr += [temp[mask].mean(0)]
        sqtrmr = torch.stack(sqtrmr)
        eye = torch.eye(sqtrmr.shape[-1]).cuda(self.cuda)/len(mus)
        sqtrmr = sqtrmr[...,None]*eye
        # Test kernel
...
#            eye = torch.eye(tk.shape[-1]).cuda(self.cuda)
            lni = (1/(lamb+self.noise+ sqtrmr+1e-8))*eye
            Qm = Km + Knm.matmul(lni).matmul(Knm.T)
            stdMat= Kmi - torch.inverse(Qm)
           
            # Covariance update
            posteriorK += [(tk - inptk.transpose(-2,-1).matmul(stdMat).matmul(inptk) + self.noise*eye+sqtrmr).slogdet()[1].cpu()]
