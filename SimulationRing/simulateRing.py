...
#    print(min(u),max(u))
    ...
    h = 1/fs
    np.random.seed(kk)
    A1 = 0.4*(np.random.randn(25,25))
    A2 = 0.4*(np.random.randn(25,25))
    A3 = 0.4*(np.random.randn(25,25))
    A4 = 0.4*(np.random.randn(25,25))
    A = np.eye(100)
    A[:25,:25] = A1
    A[25:50,25:50] = A3
    A[50:75,50:75] = A1
    A[75:,75:] = A3
    B1 = 0.4*(np.random.randn(25,25))
    B2 = 0.4*(np.random.randn(25,25))
    B3 = 0.4*(np.random.randn(25,25))
    B4 = 0.4*(np.random.randn(25,25))
...
#    U2[:,25:75] *= .1
    ...
    taus = 0.65 + np.random.randn(100)*0.015
    tauf = 0.41 + np.random.randn(100)*0.002
    taus = 0.65 + np.random.randn(100)*0.015
    tauo = 0.98 + np.random.randn(100)*0.0568
    alpha = 0.32 + np.random.randn(100)*0.0015
    Eo = 0.34 + np.random.randn(100)*0.0024
    tauv = 30
...
#        dq = F/tauo *(E/Eo - Q/V) + 1/tauv*(F - V**(1/alpha))*Q/V
    ...
    sampEns = [] 
    for SNR in [5]:
        qcons = (qpure-qpure.mean())/qpure.std()#+10**(-SNR/20)*np.random.randn(len(u),100)
        pcons = (ppure-ppure.mean())/ppure.std()#+10**(-SNR/20)*np.random.randn(len(u),100)
        b,a = signal.butter(3,.2/(fs/2), "low")
        q = signal.filtfilt(b,a,qcons,axis=0)
        p = signal.filtfilt(b,a,pcons,axis=0)
    np.save("control_2Ring{0:d}_{1:d}.npy".format(kk,conn),u)
    np.save("qpure{0:d}_{1:d}.npy".format(kk,conn),qpure)
    np.save("xpure{0:d}_{1:d}.npy".format(kk,conn),X)
    return q
for i in range(4,5):
    sampEns = np.array(Parallel(n_jobs=100)(delayed(simulate)(k,i) for k in range(100)))
    np.save("2Ring_" + str(i),sampEns)
