...
def genManifs(i,j):
    ...
    PPI = np.load("control{0:d}.npy".format(i))[500::20]
    ...
    if j == 0:
        PPItemp = np.copy(PPI)
#        PPItemp[PPI > 0] = abs(PPI[PPI > 0])
#        PPItemp[~(PPI > 0)] = -1*abs(PPI[~(PPI > 0)])
#        PPI = np.copy(PPItemp)#2*((PPI > 0).astype(float)-.5)
...
if __name__ == "__main__":
    ...
    with Pool(8) as p:
        ...
        for i in range(10):
            ...
            pvalss += [pvals]
            resss += [ress]
            np.save("PvalsPPIdeconv3.npy", np.array(pvalss).reshape(i+1,100,2))
            np.save("ResPPIdeconv3.npy", np.array(resss).reshape(i+1,100,2,2,-1))
    
    #pvals = np.array(pvalss)
    #ress = np.array(resss)
    #np.save("2GeoDoublePvalsLorenzManifold.npy", np.array(pvals).reshape(50,5))
    #np.save("2GeoDoubleResLorenzManifold.npy", np.array(ress).reshape(50,5,-1))
