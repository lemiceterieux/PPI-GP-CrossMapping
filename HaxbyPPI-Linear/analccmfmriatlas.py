...
def analCaus(dat):
    ...
    design = torch.from_numpy(design).float().cuda(cuda)
    output = torch.from_numpy(dat[2][...,None]).float().cuda(cuda)
    W = torch.linalg.inv(design.transpose(-1,-2).matmul(design)).matmul(design.transpose(-1,-2)).matmul(output)
    E = torch.var(W[...,0,:]*design[...,0] - output.squeeze(),(-1,-2))
    B = W[...,0,:].cpu().numpy()#(W[...,0,:]/E).cpu().numpy()

#    gpR = gp.GP()
#    res = gpR.testStateSpaceCorrelation(dat[0], dat[1],dat[2], m, tau=t, cuda=cuda)
    print(design.shape,output.shape,B.shape,np.sum(B>3.5),dat[-1])#,res[1][:,0], np.logical_or(res[1][:,0] < res[1].mean(1)-res[1].std(1)*2,res[1][:,0] > res[1].mean(1)+2*res[1].std(1)).sum())
    return B#res[1].cpu().numpy()
def runMVPA(d, act):
...
#    noclaim = nib.Nifti1Image(house*meanimg.get_fdata(),meanimg.affine)
    ...
    lclaim = lh*(tfh)
    rclaim = rh*(tfh)
    lnoclaim = lh*(-tfh)#house
    rnoclaim = rh*(-tfh)#house
...
#    for i in range(1,rcwp+1):
    ...
    totestno = np.array(totestno)
    np.save("Passed{0}.npy".format(d),[passedL,passedR])
    for i in range(75):
        if lcl_claim[j] is None:
            continue
        args[i] = [lcl_claim[i], totest,lmu_all[i],mus,i]
        argsno[i] = [lcl_noclaim[i] , totestno,lmu_all[i],mus,i]

    for i in range(75):
        if rcl_claim[j] is None:
            continue
        args[75+i] = [rcl_claim[i] , totest,rmu_all[i],mus,75+i]
        argsno[75+i] = [rcl_noclaim[i] ,totestno,rmu_all[i],mus,75+i]
