...
#        self.muMat = "fix" 
    ...
    def testStateSpaceCorrelation(self, X, Y, Mu, m=3, tau=1, cuda=0, inc=.7):
        ...
        xar = torch.stack([x[:,i:tau*i-tau*m] for i in range(m)],1).cuda(cuda)
        xar = xar.reshape(m,-1).T#-1,xar.shape[-1]).T
        muar = torch.stack([mu[:,i:tau*i-tau*m] for i in range(m)],1).cuda(cuda)
        muar = muar.reshape(m,-1).T#-1,muar.shape[-1]).T
        yar = []
        yp = []
        for z in y:
            yar += [torch.stack([z[:,i:tau*i-tau*m] for i in range(m)],1).cpu()]
        ...
        kx = []
        mx = []
        splits = np.array_split(np.arange(len(yar)),2)
        for s in splits:
            ytemp = yar[s,:]
            W = torch.linalg.inv(ytemp.transpose(-1,-2).matmul(ytemp)).matmul(ytemp.transpose(-1,-2)).matmul(xi)
            m, k= gpx.forward(xi, xp, ytemp.matmul(W),mup)#.matmul(s))

            mx.append(m)
            kx.append(k)
#        mx = torch.cat(mx,1).transpose(-1,-2)
        kx = torch.cat(kx,1).transpose(-1,-2)
        corr = []
        conditionals = []
        return None, kx.squeeze()
    ...
    def optimizeHyperparms(self, data, inp,mu, lr=.001, ld=1., sigma=6., noise=.1, niter=20, ns=False, kernel=None, m=40):
        ...
        eld = (ld*torch.ones(inp.shape[-1])).float().log().cuda(self.cuda).requires_grad_()
...
#            print(sld.grad.norm(), ssigma.grad, sipoints.grad.norm())
    ...
    def forward(self, inp, observations, test,mu, ld=1, sig=10, noise=.1, target="squaredexp"):
        ...
        if self.muMat is None:
            ...
            lp, parms, muMatrix, stdMatrix = self.optimizeHyperparms(observations, inp,mu, sigma=sigma, noise=noise, ld=ld ,niter=40,lr=1e-4, kernel=kernel)
            self.sigma = parms[1]
            self.ld = parms[0]
            self.noise = parms[2]
    ...
    #            time.sleep(100)
        ...
        # Predicted test vals
        posteriormu = inptk.transpose(-2,-1).matmul(self.muMat).cpu()
