...
def lorenzDrivesRossler(N=1000,eps2=0,PPI2=0, eps=.3, ylag=0, w1=1.015, w2=.985, h=.01, dnoise=0, sigma=10, beta=8/3, rho=28, initial=[1,1,1,1,1,1], PPI=1, dynamic=False):
    if np.isscalar(PPI):
        PPI = np.ones((N))
    if np.isscalar(PPI2):
        PPI2 = np.zeros((N))
    x1 = np.random.rand(N+1)*initial[0]
    x2 = np.random.rand(N+1)*initial[1]
    x3 = np.random.rand(N+1)*initial[2]

    y1 = np.random.rand(N+1)*initial[3]
    y2 = np.random.rand(N+1)*initial[4]
    y3 = np.random.rand(N+1)*initial[5]
    x1 = 1+x1**2
    x2 = 1+x2**2
    x3 = 1+x3**2
    y1 = 1+y1**2
    ...
    def dxdt(xt, yt, zt,t):
        dx = sigma*(yt - xt) +  PPI2[t]*eps2*xt*(y1[t-1] - 1)# +  eps*(x1[t-ylag] - xt)
        dy = (xt)*(rho - zt) - yt
        dz = xt*yt - beta*zt
        return np.array([dx, dy, dz])
...
#        dy1 = -w2*y2t - y3t +  eps*(.1*(x1[t-ylag]) - y1t)
        dy1 = -w2*y2t - y3t +  PPI[t]*eps*y1t*(.1*x1[t-1] - 1)
        dy2 = w2*y1t + .15*y2t
        dy3 = .2 + y3t*(y1t - 10)
        return np.array([dy1, dy2, dy3])

    for i, (x1_t,x2_t,x3_t,y1_t,y2_t,y3_t) in enumerate(zip(x1[1:-1], x2[1:-1], x3[1:-1],y1[1:-1], y2[1:-1], y3[1:-1])):
        if dynamic:
            t = i*h
        else:
            t = 1*h
        # Runge Kutta integration
        k1 = h*dxdt(x1_t,x2_t,x3_t,i)
        ...
        k4 = h*dxdt(*ink4,i)
        x1[i+2] = x1_t + 1/6*(k1[0] + k2[0]*2 + 2*k3[0] + k4[0]) + t*np.random.randn()*dnoise
        x2[i+2] = x2_t + 1/6*(k1[1] + k2[1]*2 + 2*k3[1] + k4[1]) + t*np.random.randn()*dnoise
        x3[i+2] = x3_t + 1/6*(k1[2] + k2[2]*2 + 2*k3[2] + k4[2]) + t*np.random.randn()*dnoise

        # Runge Kutta integration
        k1 = h*dydt(y1_t,y2_t,y3_t, i)
        ...
        k4 = h*dydt(*ink4, i)
        y1[i+2] = y1_t + 1/6*(k1[0] + k2[0]*2 + 2*k3[0] + k4[0]) + np.random.randn()*dnoise
        y2[i+2] = y2_t + 1/6*(k1[1] + k2[1]*2 + 2*k3[1] + k4[1]) + np.random.randn()*dnoise
        y3[i+2] = y3_t + 1/6*(k1[2] + k2[2]*2 + 2*k3[2] + k4[2]) + np.random.randn()*dnoise
    return x1,x2,x3,y1,y2,y3


def rosslerDrivesLorenz(N=1000, eps=.3, ylag=0, w1=1.015, w2=.985, h=.01, dnoise=0, sigma=10, beta=8/3, rho=28, initial=[1,1,1,1,1,1], PPI=1, dynamic=False):
    if np.isscalar(PPI):
        PPI = np.ones(len(N))
...
#        dx = sigma*(yt - xt) +  eps*((x1[t-ylag] - x1.mean()) - xt)
        dx = sigma*(yt - xt) +  PPI[t]*eps*xt*(x1[t-ylag] - 1)
        dy = (xt)*(rho - zt) - yt 
        dz = xt*yt - beta*zt
        return np.array([dx, dy, dz])
