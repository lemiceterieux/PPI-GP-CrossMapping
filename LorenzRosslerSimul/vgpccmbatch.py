...
#        self.muMat = "fix" 
    ...
    def optimizeHyperparms(self, data, inp,mu, lr=.001, ld=1., sigma=6., noise=.1, niter=20, ns=False, kernel=None, m=40):
        ...
        noise = torch.tensor(2*noise).float().cuda(self.cuda).requires_grad_()
        ...
        for i in range(niter):
            ...
            for k in range(mSamples):
                ...
                trr = train.matmul(V)
                mrr = mu.matmul(V)
                sqtrmr = noise**2*(trr-mrr)**2
                K = Knm.T.matmul(Kmi).matmul(Knm)  + lamb + ((sqtrmr).mean(1)+noiseo**2)*eye
                Kinv = torch.inverse(K)
    
                logp = 0
                for tr,mm in zip(trr.T,mrr.T):
                    logp += -.5*(tr-mm).T.matmul(Kinv).matmul(tr-mm)
...
#            print(sld.grad.norm(), ssigma.grad, sipoints.grad.norm())
        ...
        noiseo = ((sqtrmr).mean(1)+noiseo**2).detach().requires_grad_(False)
