...
def analPs(rado,mu=0):
    ...
    for R in r.T:
            with warnings.catch_warnings():
                ...
                res += [stats.wilcoxon(R, alternative='greater')]
    res = np.array(res)
...
#        implot = ts*(ts>tthresh)
    ...
    for i in range(len(rado)):
        ...
        for j in range(1):#while pvaljoints > alpha/multcomp or pvalcols > alpha/multcomp or pvalrows > alpha/multcomp:
            ...
            plt.close()
            toCalcEnt = ps<=pthresh#0.05/bonfact#/len(ts.ravel())

            tscorenull = stats.t.ppf(1-0.05/bonfact,rado.shape[2]-1)
            tscore = stats.t.ppf(1-0.05/bonfact,rado.shape[2]-1)
            resss = np.array(Parallel(n_jobs=250)(delayed(bootstrap)(rado[i],mrado[i],pthresh) for z in range(1000))).T
        ...
        for u,v,t in edgelist:
            ...
            listFDRWilcCoups += ["{0}-->{1}".format(fulllabs[u],fulllabs[v])]
            weights += [t]
        with open("FDRWilcCouplesParcs_{0}_{1}_{2}.txt".format(first[i],"",'parcs'),'w') as ff:
            for w,lcoups in zip(weights,listFDRWilcCoups):
                ff.write("{0}   {1:0.4f}\n".format(lcoups,w))
        ...
        ax.grid()
        plt.tight_layout()
        plt.savefig("FDRWilcMatrix_Ts_{0}_{1}_{2}.png".format(first[i],"",'parcs'))
        plt.close() 
        ...
        ax.grid()
        plt.tight_layout()
        plt.savefig("SkinnyFDRWilcMatrix_Ts_{0}_{1}_{2}.png".format(first[i],"",'parcs'))
        plt.close() 
    ...
    with open("SurfFDRWilcCouplesParcs_All.txt",'w') as ff:
    ...
    plt.tight_layout()
    plt.savefig("SurfFDRWilcSepColorMat")
...
#                x = 0
#                x += np.load("{3}_{1}_{0}cause.npy".format(cue,rad,cl,d))#"taste_9_claimcause.npy")#,allow_pickle=True).tolist()
#                if d == 0:
#                    x = np.median([x] + [np.load(str(ppp)+"_{3}__{1}_{0}cause.npy".format(cue,rad,cl,d))  for ppp in range(3)],0)
